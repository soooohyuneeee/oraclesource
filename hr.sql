...
-- [문제1] employees 테이블의 first_name, last_name, job_id 조회
SELECT
    *
FROM
    employees;
...
-- [문제9] 연봉이 2500, 3500, 7000이 아니며, 직업이 SA_REP, ST_CLERCK 인 사원 조회
-- 전체 정보 조회
SELECT
    *
FROM
...
-- [문제 10] '2008-02-20' ~ '2008-05-01' 사이에 고용된 사람들의 last_name, employee_id, hire_date
...
-- [문제15] last_name의 a 혹은 e가 들어가있는 사원들의 last_name 조회 후
-- last_name 오름차순 출력
SELECT
    last_name
FROM
...
-- [문제16] last_name의 a 와 e가 들어가있는 사원들의 last_name 조회 후
-- last_name 오름차순 출력
SELECT
    last_name
FROM
...
-- [문제] 메니저(manager_id)가 없는 사원들의 last_name, job_id 조회 
SELECT
    last_name,
    job_id,
    manager_id
...
-- [문제] ST_CLERTK 인 JOB_ID 를 가진 사원이 없는 부서 ID 조회
...
-- 구하여, EMPLOYEE_ID, FIRST_NAM, JOB_ID 와 함께 출력
SELECT
    employee_id,
    first_name,
    job_id,
...
WHERE
    ...
    commission_pct IS NOT
NULL;

-- [문제] first_name 이 Curtis인 사람의 first_name, last_name, phone_number,job_id 조회
...
-- job_id 조회하기. 단, job_id가 IT-PROG인 사원의 경우 프로그래머로 변경하여 출력
SELECT
    employee_id,
    first_name,
    last_name,
...
-- [문제] JOB_ID 가 AD_PRES, PU_CLERK 인 사원들의 EMPLOYEEID, FIRST_NAME, DEPARTMENT_ID
-- JOB_ID 조회하기, 단 사원명은 FISRT_NAME과 LAST_NAME을 연결하여 출력
SELECT
    employee_id,
    first_name || ' '
                  || department_id,
...
-- 8000 미만 30%, 10,000 미만 40%, 12,000미만 42%,
-- 14,000 미만 44%, 나머지 45%
SELECT
    last_name,
    salary,
...
as select distinct first_name, last_name, hire_date from employees;
